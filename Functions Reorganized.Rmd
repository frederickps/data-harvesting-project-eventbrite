...
Here are the cities with the formatting already saved. Run the function `create_url` (below) with one of the cities here to get the url. Feel free to add more cities. 
...
We created this function so that you don't have to manually enter or copy/paste the url from your browser for the cities above. 

Currently, this function will return the events page for tomorrow's events, but you can change that in the code below. 
...
This function counts the number of pages corresponding to this city's events for tomorrow. It is useful for estimating the number of events. We also used the resulting value for looping through pages. The page number can be found in the url, which makes it easy to scrape each page. 
...
This function saves all info needed for all pages of a city's events into a dataframe and exports to a CSV file. 
