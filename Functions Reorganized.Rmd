...
for (i in all_links) {
  ...
  # Extract start time
  pre_converted_time <- html |> 
  xml_find_all("//div[@class = 'date-info']//span") |> 
   xml_text() |>  
    str_extract_all("\\b(?:\\d+:\\d+(?:pm|PM|am|AM)?|(?:\\d+:)?\\d+pm|\\d+PM|\\d+am|\\d+A  M|\\d+ - \\d+:\\d+(?:pm|PM|am|AM)?|\\d+ - \\d+(?:pm|PM|am|AM)?)") %>%
    .[[1]] %>%
    paste0(collapse = " - ")  %>% 
...
```{r}
load_cities_list()
l <- create_url("barcelona")
l <- "https://www.eventbrite.es/d/spain--sevilla/events--tomorrow/"
```
...
```{r}
df <- get_event_info(l)
write_csv(df, file = "df_sevilla_20240405.csv")
```

Get pre-convert time string and convert time format

```{r}
event_html <- get_event_links(l, 1)[10] |> 
  read_html()
ts <- pre_convert_time(event_html)
ts
convert_time_format(pre_converted_time)
```

``` {r }
event.link <- "https://www.eventbrite.es/e/rescue-in-barcelona-outdoor-escape-game-tickets-707034598517?aff=ebdssbdestsearch&keep_tld=1"
html <- event.link |> read_html()
pre_converted_time <- html |> 
  xml_find_all("//div[@class = 'date-info']//span") |> 
   xml_text() |>  
    str_extract_all("\\b(?:\\d+:\\d+(?:pm|PM|am|AM)?|(?:\\d+:)?\\d+pm|\\d+PM|\\d+am|\\d+A  M|\\d+ - \\d+:\\d+(?:pm|PM|am|AM)?|\\d+ - \\d+(?:pm|PM|am|AM)?)") %>%
