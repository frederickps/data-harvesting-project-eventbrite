...
for (i in all_links) {
  ...
  shared_df[i, "Title"] <- json$name |> str_replace(pattern = "\n", replacement = " ")
  shared_df[i, "Subtitle"] <- json$description|> str_replace(pattern = "\n", replacement = " ")
  shared_df[i, "url"] <- json$url
 
  .GlobalEnv$df <- shared_df 
...
```{r write-csv}
get_event_info(l) # don't need to save it to `df` because it will automatically do it each loop
filename <- paste0("warsaw", "20240307", ".csv")
