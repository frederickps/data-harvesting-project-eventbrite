...
shared_df <- data.frame(Duration = character(num_links), 
                        Ticket_Type = character(num_links),
                        Refund_Policy = character(num_links), 
                        Description = character(num_links),
                        Start_Time = character(num_links),
                        Currency = character(num_links),
                        LowPrice = numeric(num_links),
                        HighPrice = numeric(num_links),
                        EventStatus = character(num_links),
                        Organizer = character(num_links),
                        StartTime = lubridate::as_datetime(character(num_links)),
                        EndTime = lubridate::as_datetime(character(num_links)),
                        Title = character(num_links),
                        Subtitle = character(num_links),
                        url = character(num_links),
...
for (i in all_links) {
  ...
  # shared_df[i, "Start_Time"] <- starttime
  ...
  print(paste(100 * j/num_all_links, "% completed")) 
}
...
```{r}
cities.list <- load_cities_list()
l <- create_url("warsaw")
list <- get_all_event_links(l)
html <- lapply(list, read_html) 
time <- lapply(html, pre_convert_time)
lapply(time, convert_time_format)
...
```{r}
df <- get_event_info(l)
filename <- paste0("warsaw", "20240306", ".csv")
